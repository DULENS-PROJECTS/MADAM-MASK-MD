 ...
 * @modified by : DULENS-PROJECTS <https://github.com/DULENS-PROJECTS/MADAM-MASK-MD>
 * @description : MADAM-MASK MD,A Multi-functional whatsapp bot.
 * @version 0.0.6
 **/
...
async(Void, citel, text,{ isCreator }) => {
    ...
    if(lol){
        ...
        const give2 = await eco.give(citel.sender, "MADAM-MASK-MD", deduff*2);
    ...
    const deduct1 = await eco.deduct(citel.sender, "MADAM-MASK-MD", deduff);
    let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `
            let str = st.replace(/1/g, `🔴`).replace(/2/g, `🔵`).replace(/3/g, `🟣`).replace(/4/g, `🟢`).replace(/5/g, `🟡`).replace(/6/g, `⚪️`).replace(/7/g, `⚫️`).replace(/:/g, `    `)
